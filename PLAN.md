...
Phase 5 - Node management (closing out 3 administrative items),
then Phase 18 - Pipeline DSL + GHPRB master-job execution.
...
- [ ] 5.2 Failing test (tests/agent_e2e_test.go): an agent
        ...
        Closing-out note: the spec calls for a file under tests/
        but the equivalent coverage already lives in
        internal/agentsvc/connect_test.go. To resolve, either
        (a) add tests/agent_e2e_test.go that exec's a real
        cmd/wolfci-agent subprocess, or (b) update the spec to
        cite the internal test as the gate and move this item to
        PLAN.historic.
...
- [ ] 5.5 Scheduler + Provisioner integration: when a Job's
        ...
        Closing-out note: same situation as 5.2 - the
        equivalent coverage lives in
        internal/scheduler/router_test.go. Resolve by either
        adding the tests/scheduler_provisioner_test.go wrapper
        or moving this item to PLAN.historic and citing the
        internal test as the gate.
